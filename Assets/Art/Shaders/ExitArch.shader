// 탈출구 전용 아치 실루엣 — 위 반원 + 아래 사각(바닥 평평), 폭의 절반이 아치 반지름.
// 렌더만 아치로 바꾸고 콜라이더/트리거는 그대로 둔다(판정 무변경 — 쿼드 안에서 그리는 방식).
// 보일·테두리 파라미터는 4종 공통 블록과 같은 의미(2026-08-17 통일 체계). _CornerRadius는
// 아치 실루엣에선 미사용(블록 순서 유지용) — 바닥 모서리는 접지라 각져도 자연스럽다.
Shader "AfterYou/ExitArch"
{
    Properties
    {
        ...
        _CornerRadius ("Corner Radius (unused for arch)", Range(0, 16)) = 4.7
        ...
        _HandleRadius ("Handle Radius (x width)", Range(0, 0.3)) = 0.09
        _HandleX ("Handle X (0=left, 1=right)", Range(0, 1)) = 0.22
        _HandleY ("Handle Y (x height)", Range(0, 1)) = 0.45
    }
    ...
    SubShader
    {
        ...
        {
            HLSLPROGRAM
            #pragma vertex ExitArchVertex
            #pragma fragment ExitArchFragment
            ...
            Varyings ExitArchVertex(Attributes input)
            {
            ...
            half4 ExitArchFragment(Varyings input) : SV_Target
            {
                ...
                // 콜라이더 기준 픽셀 좌표계(원점 = 좌하단). fwidth 역수 = 콜라이더 px 크기라
                // 비균등 스케일에서도 아치 원이 진짜 원으로 나온다.
                float2 pPx = uvC / fw;
                float2 sizePx = 1.0 / fw;

                // 아치 SDF: 반지름 = 폭의 절반, 원 중심 = 꼭대기에서 반지름만큼 아래.
                float halfW = sizePx.x * 0.5;
                float R = halfW;
                float cy = max(sizePx.y - R, 0.0);
                float xC = pPx.x - halfW;

                // 몸통 사각(0..cy) + 상단 반원의 합집합(min).
                float2 db = abs(float2(xC, pPx.y - cy * 0.5)) - float2(halfW, cy * 0.5);
                float sdRect = length(max(db, 0.0)) + min(max(db.x, db.y), 0.0);
                float sdCirc = length(float2(xC, pPx.y - cy)) - R;
                float edgePx = -min(sdRect, sdCirc); // 양수 = 내부
                ...
                // 손잡이: 좌측 원 1개, 테두리 색과 동일(몸통 색 x darken). 같은 wobble을 먹여
                // 문 실루엣과 한 몸으로 보인다. 아치 밖으로 삐져나가는 부분은 alpha 0이라 무해.
                float2 handleC = float2(_HandleX * sizePx.x, _HandleY * sizePx.y);
                float sdHandle = length(pPx - handleC) - _HandleRadius * sizePx.x;
                half handleMask = saturate(-sdHandle + wobble * _WobbleAmp + 0.5);
                rgb = lerp(rgb, tint.rgb * _OutlineDarken, handleMask);
