...
- **아트 디렉션 확정 · 문서화 완료** (2026-07-25) — "테이프/방송 세계" 콘셉트, `Docs/team/ART-DIRECTION.md` 신규. 정체성 4색 팔레트 확정(Heavy 인디고/Climber 마젠타 퍼플/Carrier 앰버/Light 민트 시안, 명도 사다리 포함). **에셋 미적용** — Identity_*.asset 4색 반영은 Step 2(하네스 1회: 4색+Volume+클론 스캔라인 셰이더+REC 오버레이)에 포함
...
4. **[비차단·임의 시점] 아트 Step 2 룩 슬라이스** (하네스 1회) — Identity 에셋 4색 적용 + Global Volume + 클론 스캔라인 셰이더 + REC 오버레이. 상세는 `Docs/team/ART-DIRECTION.md` §6
