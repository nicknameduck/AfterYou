...
## 총점 규칙 (만점 방지)
- 배점 합계 100점: 컨벤션 12 + 생명주기 9 + 성능 10 + 안전성 9 + 기능 충족 23 + 적대 검증 17 + 단순성 10 + 완결성 10
- **95점 이상은 다음 두 조건을 모두 만족할 때만 부여한다**: (1) 적대 검증 시나리오 3개 전부 실측 통과 (2) 스펙 감사 무결(체크리스트 누락 0건). 하나라도 미달이면 총점 상한 94점.
...
조건 누락 1개당 -5점, 최대 -23점.
**적대 검증(반증 실측)을 수행하지 않은 경우 이 섹션의 상한은 50%(12점)다** — happy path 확인만으로는 충족을 증명할 수 없다.
...
"조건이 충족되는 근거 찾기"(확인)가 아니라 **"조건을 깨뜨리는 입력 찾기"(반증)**를 채점한다. 스펙 위반이나 상태 오염을 유발할 만한 적대 시나리오 **3개**를 설계하고 MCP로 실측한다.

### 시나리오 설계 기준
- 스펙의 경계·전이·동시성에서 고른다. 예: 상태 전환 중 인터럽트(재생 중 Rewind), 수량 경계(오브젝트 0개/2개 이상), 순서 역전(의도된 순서의 반대 시도), 리소스 재사용(리셋 후 재진입)
- happy path의 재탕(정상 절차를 그대로 수행하는 시나리오)은 인정하지 않는다

### 채점
- 시나리오 1개당 5~6점 (3개 합계 17점): 설계 타당성 + 실측 수행 + 결과 기록이 모두 있어야 만점
- **실측 미수행** 시나리오 → 해당 시나리오 0점
- **피상적 시나리오**(happy path 재탕, 코드만 읽고 "문제없음" 판정) → 해당 시나리오 -3점
- 시나리오가 실제 결함을 발견한 경우 이 섹션은 감점하지 않는다 (채점기가 일한 것). 발견된 결함은 기능 충족·회귀 방지에서 감점한다

### 결과 형식
```
### 적대 검증
[시나리오 1] 재생 중 Rewind 인터럽트 — 클론 재생 3틱 시점에 Rewind 호출
  → 실측: box.Mode=Frozen 복귀, OwnerSlot=-1 ✓ 통과
[시나리오 2] ...
  → 실측: ... ✗ 위반 발견 → [Major]로 보고
```
